...
library(NbClust)
set.seed(1234)
nc <- NbClust(data_wine, min.nc=2, max.nc=15, method="kmeans")
...
table(wine$Type)
table_wine <- table(wine$Type, fit.km$cluster) # fit.km$cluster is the row entries
table_wine 

library(flexclust)
randIndex(table_wine)
...
clusplot( data_wine, fit.km$cluster, color = T, shade = T, 
